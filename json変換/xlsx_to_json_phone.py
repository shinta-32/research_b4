...
# 使用する列の範囲を定義
intensity_columns = [f'Answer.intensity_{i}' for i in range(5, 29)]
image_url_columns = [f'Input.image_url_{i}' for i in range(5, 29)]

# 相対パスで保存先のディレクトリを指定
output_directory = './exp_2412/phone/'
...
for i in range(len(df)):
    ...
    for intensity_col, image_col in zip(intensity_columns, image_url_columns):
        ...
        if hue is not None and saturation is not None and value is not None:
            # 新しい画像ファイル名をHSV値で作成
            new_image_filename = f"images/phone-{hue}-{saturation}-{value}.jpg"

            # 嗜好度をint型に変換して辞書に保存
            result[new_image_filename] = int(intensity_value)

    # 個別のJSONファイルを保存
    output_path = os.path.join(output_directory, f"{worker_id}_phone.json")
